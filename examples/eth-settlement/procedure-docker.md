...
- Notice what `-p` options are doing.
    1. Containers have the same local port `3000`.
    1. On the other hand, the ports are mapped to local ports of `3000`, `3001`.
- They are connected to the network `interledger`.
    - So they can communicate each other using host name of `interledger-rs-se_a`, `interledger-rs-se_b` and `redis`.
- `--redis_uri` specifies where Redis is located, and the host name is `redis` which will be resolved to the container named `redis`.
